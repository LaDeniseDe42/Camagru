...
class Publication
{
    ...
    public function deleteComment($commentId, $userId)
    {
        ...
        if ($stmt->rowCount() > 0) {
            error_log("Suppression réussie du commentaire $commentId par user $userId");
            return ['success' => true, 'message' => 'Commentaire supprimé avec succès.'];
        } else {
            error_log("Échec suppression : commentaire $commentId inexistant ou non autorisé pour user $userId");
    ...
    public function addComment($userId, $publicationId, $content)
    {
        ...
        if ($stmt->rowCount() > 0) {
            $commentId = $this->dbConnection->lastInsertId();
            return [
                'success' => true,
